...
uses
  ...
  SynEdit, SynEditHighlighter, SynHighlighterPas, StdCtrls, SynMemo, ExtCtrls,
  ComCtrls, SynEditTypes, Menus, math, uPSComponent, uPSUtils, uPSRuntime,
  SynCompletion, uPSComponent_Default, uPSComponent_StdCtrls, uPSI_user_charts,
  uPSComponent_Controls, uPSComponent_Forms, SynEditMiscClasses, uPSCompiler,
  Clipbrd, lNetComponents, IniPropStorage, SynEditMarks, PrintersDlgs, process,
  LCLIntf, Types, LCLType, lnet, dynmatrix, gchannels, user_charts;
...
type
  ...
  private
    ...
    //procedure PubZMQData(s: string; multiPart: boolean);
    //function ReadUDPData: string;
    //function ReadZMQData: string;
...
uses main, uPSDebugger, Utils, uPSI_dynmatrix;
...
begin
  ...
  if (LinCol.x<>-1) and (LinCol.y<>-1) then begin
    ...
    SynEditST.caretY:=LinCol.y - SynMemoHeader.Lines.Count - 1;
...
begin
  ...
  ProgTime := ProgCyclesCount * 0.04; //TODO
  ...
  StatusBar.Panels[3].Text := format('%f us',[1e6*(i64_end - i64_start)/i64_freq]);
...
var Plugin, PathPlugin, MatrixPlugin, ChartPlugin: TPSPlugin;
  s: string;
begin
  FormEditorCaption := FEditor.Caption;
  IniPropStorage.IniFileName := 'editor.ini'; //GetIniFineName(copy(name, 2, MaxInt));
...
var i: integer;
    s: string;
begin
  Sender.AddFunction(@gMessageFloat, 'procedure gMessageFloat(c: string; f: single)');
  Sender.AddFunction(@gMessageInt, 'procedure gMessageInt(c: string; i: integer)');
  Sender.AddFunction(@gMessageStr, 'procedure gMessageStr(c: string; s: string)');

  Sender.AddFunction(@arcsin, 'function arcsin(x: double): double');
  Sender.AddFunction(@arccos, 'function arccos(x: double): double');
...
//  Sender.AddMethod(Self, @TFEditor.myformat, 'function Format(const sFormat: string; const Args: array of const): string;');
  ...
  Sender.AddMethod(Self, @TFEditor.PubZMQData, 'procedure PubZMQData(s: string; multiPart: boolean);');
  Sender.AddMethod(Self, @TFEditor.ReadZMQData, 'function ReadZMQData: string;');}
  ...
  Sender.AddFunction(@ButtonPressed, 'function ButtonPressed(ButtonName: string): boolean;');
...
begin
  if (Line = LocalInspectorLine) and ProgRunning then begin
    Special := True;
    BG := cllime;
    FG := clRed;
  end;
...
procedure TFEditor.PubZMQData(s: string; multiPart: boolean);
begin
  if not FParams.ZPub.Active then exit;
  FParams.ZPub.WriteData(s, multiPart);
end;

function TFEditor.ReadZMQData: string;
begin
  result := '';
  with FParams do begin
    if ZMQPackets.Count > 0 then begin
      result := ZMQPackets.Text;
      ZMQPackets.Clear;
    end;
  end;
end;   }
